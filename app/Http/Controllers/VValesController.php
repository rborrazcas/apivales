...
class VValesController extends Controller
{
    function getSearchFolio(Request $request){
        ...
        try {
            ...
            ->select(
                'VS.id', 'VS.Ejercicio', 'VS.idSolicitud', 'VS.CURP', 'VS.Nombre', 'VS.Paterno', 'VS.Materno', 
                'VS.CodigoBarrasInicial', 'VS.CodigoBarrasFinal', 'VS.SerieInicial', 'VS.SerieFinal', 'VS.Articulador', 'VS.Municipio', 'VS.Remesa', 'VS.created_at', 'VS.UserCreated', 
                'VS.updated_at', DB::raw("concat_ws(' ',U.Nombre, U.Paterno, U.Materno) AS Capturo")
            )
            ...
            return ['success'=>true,'results'=>true, 'total'=>$total, 'data'=>$res];
    ...
    function getValesAvances2021(Request $request){
        ...
        try {
            ...
            $tablaMeta = "(
              ...
              ->selectRaw("M.Region, M.Municipio, M.Apoyos, AC.AprobadosComite, if(VI.Incidencias is null, 0, VI.Incidencias) as Incidencias, (M.Apoyos + if(VI.Incidencias is null, 0, VI.Incidencias) - if(AC.AprobadosComite is null, 0, AC.AprobadosComite)) as ApoyosMenosApronadosComite, if(ET.Entregados is null, 0, ET.Entregados) as Entregados, S.SolicitudesPorAprobar, E.ExpedientesRecibidos")
            //   ->leftJoin(DB::raw($tablaMeta),'M.idMunicipio','=','M.Id')
              ...
              ->leftJoin(DB::raw($tablaIncidencias),'VI.idMunicipio','=','M.Id');
            ...
            $queryGeneral->orderBy('M.Region', 'ASC')
    ...
    function getValesV2Fecha(Request $request){
        ...
        try {
            ...
            if(isset($parameters['Folio'])){
                $valor_id = $parameters['Folio'];
                $res->where(DB::raw('LPAD(HEX(vales.id),6,0)'),'=',''.$valor_id.'');
            }
    ...
    function getValesV2(Request $request){
        ...
        DB::table('users_filtros')->where('UserCreated',$userId)->where('api','getValesV2')->delete();   
        ...
        try {
            ...
            if($total==0) return['success'=>true,'results'=>true,
            'total'=>$total,'filtros'=>$parameters['filtered'],'data'=>$array_res];
            ...
            foreach($res as $data){
                $temp = [
                    ...
                    'CelularUserDocumentacion'=> $data->CelularUserDocumentacion,
                    'Remesa'=> $data->Remesa,
                    'idLocalidad'=> [
                        'id'=> $data->idLocalidad,
                        'Nombre'=> $data->Localidad,
    ...
    function getValesRegion(Request $request){
        ...
        try {
            ...
            ->select(
                ...
                DB::raw('concat_ws(" ",users.Nombre, users.Paterno, users.Materno) AS Capturo'),
                DB::raw('concat_ws(" ",UO.Nombre, UO.Paterno, UO.Materno) AS Articulador'),
                'vales.idStatus',
                'vales_status.Estatus'
            )
    ...
    function getValesResumen(Request $request){
        ...
        DB::table('users_filtros')->where('UserCreated',$userId)->where('api','getValesResumen')->delete();   
        ...
        try {
            if(isset($parameters['UserOwned'])){
                ...
                $res = DB::table(DB::raw('
                    ...
                    where V.isEntregado=1 and V.idIncidencia=1 and V.UserOwned='.$userId.'
                    ...
                    where V.idIncidencia !=1 and V.UserOwned='.$userId.'
                ...
                $SinExpedientes = DB::select('SELECT
                count(isDocumentacionEntrega) as SinExpediente
                FROM
                sedeshu.vales
                where year(FechaSolicitud)>=2021 and isDocumentacionEntrega=0 and UserOwned = '.$userId);

                $NoEntregados = DB::select('SELECT
                count(isEntregado) as NoEntregados
                FROM
                sedeshu.vales
                WHERE
                year(FechaSolicitud)>=2021 and isEntregado=0 and UserOwned = '.$userId
                );
                $response = ['success'=>true,'results'=>true, 
                'total'=>count($res),'filtros'=>$parameters, 'data'=>$res,
                'resumen'=>[
                    'SinExpediente'=>$SinExpedientes[0]->SinExpediente,
                    'NoEntregados'=>$NoEntregados[0]->NoEntregados
                ],
                'errors'=>'¡Sin errores!', 'message' =>'¡Consulta exitosa!'];
                ...
                $response = ['success'=>true,'results'=>false, 
                'total'=>0,'filtros'=>[],
                'errors'=>'No se recibio el campo UserOwned', 'message' =>'¡Ocurrio un error contacte al administrador!'];
            ...
            'total'=>0,'filtros'=>[],
            'errors'=>$e->getMessage(), 'message' =>'¡Ocurrio un error contacte al administrador!'];
    ...
    function setVales(Request $request){
        ...
        if ($v->fails()){
            $response =  ['success'=>false,'results'=>false,
            'errors'=>$v->errors(),'data'=>[], 'message'=>'¡Error de validación: '.$v->errors()->first().'!'];
        ...
        switch ($user->email) {
            ...
            case '4686895105':
            case '4686897060':
                if(!in_array($parameters['idMunicipio'], [6, 13, 32, 33, 34, 40, 43, 45])){ //R1
                    $response =  ['success'=>true,'results'=>false,
                    'errors'=>[],'data'=>[], 'message'=>'¡Error de Captura: Municipio Restringido!'];
                    return response()->json($response,200);
                }  
            ...
            case '4721272959':
            case '4721653288':
                if(!in_array($parameters['idMunicipio'], [15, 17, 25, 26, 27, 31, 37])){ //R3
                    $response =  ['success'=>true,'results'=>false,
                    'errors'=>[],'data'=>[], 'message'=>'¡Error de Captura: Municipio Restringido!'];
                    return response()->json($response,200);
                }  
            ...
            case '4431423672':
            case '4291226344':
                if(!in_array($parameters['idMunicipio'], [1, 8, 12, 16, 23, 24, 42])){ //R4
                    $response =  ['success'=>true,'results'=>false,
                    'errors'=>[],'data'=>[], 'message'=>'¡Error de Captura: Municipio Restringido!'];
                    return response()->json($response,200);
                }  
            ...
            default:
            $response =  ['success'=>true,'results'=>false,
            'errors'=>[],'data'=>[], 'message'=>'¡Error de Captura: No cuenta con permisos para realizar capturas de vales!'];
            return response()->json($response,200);
                break;
        }
        ...
        if(DB::table('users_cancelados')->where('idUser',$user->id)->first()){
            $response =  ['success'=>true,'results'=>false,
            'errors'=>$v->errors(),'data'=>[], 'message'=>'¡Error de permisos: No tiene derechos para ejecutar esta accion, contacte al administrador!'.$v->errors()->first().'!'];
        }
        ...
        ->whereRaw('year(vales.FechaSolicitud) = 2021')->get();
        if(count($validacion_curp) > 1){
            return ['success'=>true,'results'=>false,
                'data'=>[],'message'=>'Error CURP: ¡La CURP '.($parameters['CURP']).' ya cuenta con mas de 2 solicitudes!'];
        }
        if($parameters['NumeroPersonas']<= 0){
            return ['success'=>true,'results'=>false,
                'data'=>[],'message'=>'Error en el número de personas: ¡El número de personas debe ser 1 o mayor!'];

        }
        
        $validar_monto = DB::table('config')->first();
        $ambito_localidad = DB::table('et_cat_localidad')->where('et_cat_localidad.Id','=',$parameters['idLocalidad'])->first();
        $parameters['TotalIngresos'] = floatval($parameters['IngresoPercibido']) + floatval($parameters['OtrosIngresos']);

        if (strcmp ($ambito_localidad->Ambito, 'U' ) == 0) {
            if(($parameters['TotalIngresos']/$parameters['NumeroPersonas'])>$validar_monto->MontoUrbano){
                return ['success'=>true,'results'=>false,
                'data'=>[],'message'=>'Error: ¡El total de ingresos por persona excede el monto permitido en la localidad urbana!'];
            }
        }
        else{
            if(($parameters['TotalIngresos']/$parameters['NumeroPersonas'])>$validar_monto->MontoRural){
                return ['success'=>true,'results'=>false,
                'data'=>[],'message'=>'Error: ¡El total de ingresos por persona excede el monto permitido en la localidad rural!'];
            }
        }
        ...
        // $validar_espacio = DB::select('Select M.region,  M.idMunicipio, M.Municipio, M.ApoyoAmpliado, A.Total as Avance, if(A.Total>=M.ApoyoAmpliado, 0, 1) as Captura
        // From
        // (select * from meta_municipio where Ejercicio='.$parameters['Ejercicio'].' and  idMunicipio = '.$parameters['idMunicipio'].') as M
        // left join 
        // (select count(id) Total, idMunicipio from vales where (idStatus=1 or idStatus=5) and idIncidencia=1   
        // and YEAR(FechaSolicitud)='.$parameters['Ejercicio'].' and  idMunicipio = '.$parameters['idMunicipio'].'
        // group by idMunicipio) A
        // on (A.idMunicipio = M.idMunicipio)');

        $validar_espacio = DB::select('Select M.region,  M.idMunicipio, M.Municipio, M.ApoyoAmpliado, A.Total as Avance, if(A.Total>=M.ApoyoAmpliado, 0, 1) as Captura
        From
        (select * from meta_municipio where Ejercicio= 2021 and  idMunicipio = '.$parameters['idMunicipio'].') as M
        left join 
        (select count(id) Total, idMunicipio from vales where (idStatus=1 or idStatus=5) and idIncidencia=1   
        and YEAR(FechaSolicitud)= 2021 and  idMunicipio = '.$parameters['idMunicipio'].'
        group by idMunicipio) A
        on (A.idMunicipio = M.idMunicipio)');

        if(!$validar_espacio) return ['success'=>true,'results'=>false,
        'data'=>[],'message'=>'Error: El municipio no tiene definida una meta de vales para el Ejercicio '.$parameters['Ejercicio']];
        ...
        if($validar_espacio[0]->Captura == 1){
            ...
            ->where('vales.id','=',$vale_->id)->first();
                return ['success'=>true,'results'=>true, 'data'=>$vale];
        }
        else{
            return ['success'=>true,'results'=>false,
            'data'=>[],'message'=>'Error: ¡El municipio ya excedió el número de solicitudes!'];
        }
    }
    ...
    function updateVales(Request $request){
        ...
        if($vale_bloqueado == 1){
            ...
            return ['success'=>true,'results'=>false,
                        'data'=>[],'message'=>'Error Vale Bloqueado: ¡No se puede modificar los valores del vale, póngase en contacto con el administrador!'];
            ...
            if(isset($parameters['IngresoPercibido']) &&
            isset($parameters['OtrosIngresos'])
            ){
                if(!isset($parameters['idLocalidad'])){
                    $parameters['idLocalidad'] = $vale = VVales::find($parameters['id'])->idLocalidad;
                }
                if(!isset($parameters['NumeroPersonas'])){
                    $parameters['NumeroPersonas'] = $vale = VVales::find($parameters['id'])->NumeroPersonas;
                }
                $validar_monto = DB::table('config')->first();
                $ambito_localidad = DB::table('et_cat_localidad')->where('et_cat_localidad.Id','=',$parameters['idLocalidad'])->first();
                $parameters['TotalIngresos'] = floatval($parameters['IngresoPercibido']) + floatval($parameters['OtrosIngresos']);
                ...
                $parameters['TotalIngresos'] = floatval($parameters['IngresoPercibido']) + floatval($vale->OtrosIngresos);
                if (strcmp ($ambito_localidad->Ambito, 'U' ) == 0) {
                    if(($parameters['TotalIngresos']/$parameters['NumeroPersonas'])>$validar_monto->MontoUrbano){
                        return ['success'=>true,'results'=>false,
                        'data'=>[],'message'=>'Error: ¡El total de ingresos por persona excede el monto permitido en la localidad urbana!'];
                ...
                $ambito_localidad = DB::table('et_cat_localidad')->where('et_cat_localidad.Id','=',$parameters['idLocalidad'])->first();
                $vale = VVales::find($parameters['id']);
                $parameters['TotalIngresos'] = floatval($parameters['OtrosIngresos']) + floatval($vale->IngresoPercibido);
            ...
            $vale = VVales::find($parameters['id']);
            $vale_insert=DB::table('vales')->select('vales.*','vales.id as idVale',DB::raw($user->id. " as UserEdito, '".date('Y:m:d h:m:s')."' as UserEditoFecha"))
    ...
    function updateRecepcionDocumento(Request $request){
        ...
        //else lo normal;
        //Validacion nueva tambien la coloco en el update para que no quieran modificar
        $vale_bloqueado = VVales::find($parameters['id'])->isDocumentacionEntrega;

        //dd($vale_bloqueado);
        if($vale_bloqueado == 1){

            return ['success'=>true,'results'=>false,
                        'data'=>[],'message'=>'Error Vale Recepcionado: ¡No se puede recepcionar nuevamente, póngase en contacto con el administrador!'];
    ...
    function getSolicitudesPorCURP(Request $request ){
        ...
        ->select(
            ...
            ->where('CURP',$parameters['CURP'])->get();
    ...
    function getHistoryVales(Request $request){
        ...
        try {
            ...
            ->select(
                'vales_history.id', 
                DB::raw('LPAD(HEX(vales_history.idVale),6,0) as ClaveUnica'),
            ...
            ->leftJoin('et_cat_municipio','et_cat_municipio.Id','=','vales_history.idMunicipio')
            ->leftJoin('et_cat_localidad','et_cat_localidad.Id','=','vales_history.idLocalidad')
            ->leftJoin('vales_status','vales_status.id','=','idStatus')
            ->leftJoin('users','users.id','=','vales_history.UserCreated')
            ->leftJoin('cat_usertipo','cat_usertipo.id','=','users.idTipoUser')
            ->leftJoin('users as usersB','usersB.id','=','vales_history.UserUpdated')
            ->leftJoin('cat_usertipo as cat_usertipoB','cat_usertipoB.id','=','usersB.idTipoUser')
            ->leftJoin('users as usersC','usersC.id','=','vales_history.UserOwned')
            ->leftJoin('users as usersCretaed','usersCretaed.id','=','vales_history.UserCreated')
            ->leftJoin('users as userEdito','userEdito.id','=','vales_history.UserEdito')
            ->leftJoin('cat_usertipo as cat_usertipoC','cat_usertipoC.id','=','usersC.idTipoUser');
            
            if(isset($parameters['Folio'])){
                $valor_id = $parameters['Folio'];
                $res->where(function($q)use ($valor_id) {
                    $q->where(DB::raw('LPAD(HEX(vales_history.idVale),6,0)'),'like','%'.$valor_id.'%');
    ...
    function getValesInHistory(Request $request){
        ...
        try {
            ...
            ->leftJoin('users as usersC','usersC.id','=','vales.UserOwned')
            ->leftJoin('users as usersCretaed','usersCretaed.id','=','vales.UserCreated')
            ->leftJoin('vales_history as valesHist','valesHist.idVale','=','vales.id')
            ->leftJoin('users as userEdito','userEdito.id','=','valesHist.UserEdito')
            ->leftJoin('cat_usertipo as cat_usertipoC','cat_usertipoC.id','=','usersC.idTipoUser')
    ...
    function getValesFecha(Request $request){
        ...
        try {
            ...
            ->leftjoin('et_cat_municipio as m','m.Id','=','vales.idMunicipio')
            ...
            $array_res = [];
            $temp = [];
            $temp_axu =['FechaAlta'=>'Seleccionar...','FechaHoraAlta'=>'Seleccionar...'];
            foreach($res as $data){
                $temp = [
    ...
    function deleteVales(Request $request){
        ...
        try {
            ...
            ->where('id',$parameters['id'])->first();
            if(!$vale){
                return ['success'=>true,'results'=>false,'data'=>[],'message'=>'¡EL vale no se encontró, contacte al administrador!'];
            }
            if($vale->Remesa){
                return ['success'=>true,'results'=>false,'data'=>[],'message'=>'¡EL vale no se puede eliminar con una remesa asignada!'];
            }
            ...
            DB::table('vales')->where('id',$parameters['id'])->delete();
            ...
            $response = ['success'=>true,'results'=>false, 
            'total'=>0,
            'errors'=>$e->getMessage(), 'message' =>'¡Ocurrio un problema, contacte al administrador!'];

...
class VNegociosController extends Controller
{
                ...
                for($i=0;$i<count($parameters['filtered']);$i++){
                    ...
                    if($flag==0){
                        switch ($parameters['filtered'][$i]['id']) {
                            ...
                            break;
                            case 'Region':
                                $res->where('M.SubRegion','LIKE','%'.$parameters['filtered'][$i]['value'].'%');   
                            break;
                            case 'idTipoNegocio':
                                if(is_array ($parameters['filtered'][$i]['value'])){
        ...
        try {
            ...
            if(isset($parameters['idTipoNegocio'])){
                if(is_array ($parameters['idTipoNegocio'])){
                    $res->whereIn('N.idTipoNegocio',$parameters['idTipoNegocio']);
                }else{
                    $res->where('N.idTipoNegocio','=',$parameters['idTipoNegocio']);
                }    
            }
            if(isset($parameters['idTipoGiro'])){
                $res->join('v_negocios_giros','v_negocios_giros.idNegocio','N.id');
                $res->join('v_giros','v_giros.id','v_negocios_giros.idGiro');
                if(is_array ($parameters['idTipoGiro'])){
                    $res->whereIn('v_negocios_giros.idGiro',$parameters['idTipoGiro']);
                }else{
                    $res->where('v_negocios_giros.idGiro','=',$parameters['idTipoGiro']);
                }    
            }
            if(isset($parameters['Folio'])){
                $valor_id = $parameters['Folio'];
                        ...
                        if($flag==0){
                            ...
                            if($parameters['tipo']=='and'){
                                ...
                                switch ($parameters['filtered'][$i]['id']) {
                                    ...
                                    break;
                                    case 'Region':
                                        $res->orWhere('M.SubRegion','LIKE','%'.$parameters['filtered'][$i]['value'].'%');   
                                    break;
                                    case 'idTipoNegocio':
                                        if(is_array ($parameters['filtered'][$i]['value'])){
                                            $res->orWhereIn('idTipoNegocio',$parameters['filtered'][$i]['value']);
    ...
    function getNegociosPublico(Request $request){
        ...
        try {
            ...
            ->select(
                ...
                'Apis')
                ->where('idUser','=',$user->id)
                ->where('Apis','=','getNegociosPublico')->first();
    ...
    function getNegociosResumen(Request $request){
        ...
        try {
            ...
            ->leftJoin('v_negocios_tipo as T','N.idTipoNegocio','=','T.id')
            ->where('N.idStatus','=',3) //zincri
            ->groupBy('N.idTipoNegocio');
            if(isset($parameters["idMunicipio"])){
                if(is_array ($parameters['idMunicipio'])){
                    $resTipo->whereIn('N.idMunicipio',$parameters['idMunicipio']);
                }else{
                    $resTipo->where('N.idMunicipio','=',$parameters["idMunicipio"]);
                }  
            ...
            if(isset($parameters["idMunicipio"])){
                if(is_array ($parameters['idMunicipio'])){
                    $resResumen2->whereIn('N.idMunicipio',$parameters['idMunicipio']);
                }else{
                    $resResumen2->where('N.idMunicipio','=',$parameters["idMunicipio"]);
                }  
            ...
            $resTotal = DB::table('v_negocios as N')
            ->select( DB::raw('count(N.id) as Total'));
            ...
            return ['success'=>true,'results'=>true, 'total'=>$resTotal[0]->Total, 'data'=>$dataRS];
    ...
    function setNegocios(Request $request){
        ...
        try {
        $empresa_recibida = 
            $parameters['NombreEmpresa'].$parameters['idMunicipio'].$parameters['Nombre'].$parameters['Paterno'].$parameters['Materno'].$parameters['Calle'];
        $empresa_recibida = str_replace(" ","",$empresa_recibida);
        $res = DB::table('v_negocios')->select(
                'id',
                DB::raw("
                REPLACE(
                CONCAT(
        ...
        for ($i=0; $i < $res->count();  $i++) { 
            if(strcasecmp($empresa_recibida, $res[$i]->NombreCompleto) === 0){
                $flag=true;
        ...
        if($flag){
            $errors = [
                "Clave"=>"La empresa que decea registrar ya se encuentra registrada."
            ];
            $persona_existente = VNegocios::find($id_existente); 
            $response = ['success'=>false,'results'=>false, 'errors'=>$errors,
            'message'=>'La empresa que decea registrar ya se encuentra registrada.', 'Empresa Existente'=>$persona_existente];
        ...
        $res_giros = DB::table('v_negocios_giros')
            ...
            ->leftJoin('v_giros','v_giros.id','=','v_negocios_giros.idGiro')
            ->where('v_negocios_giros.idNegocio','=',$negocio->id)->get();
        ...
        return ['success'=>true,'results'=>true,
            ...
            $response = ['success'=>true,'results'=>false, 
            'total'=>0,
            'errors'=>$errors, 'message' =>'Error Interno de Query'];
    ...
    function updateNegocios(Request $request){
        ...
        if(isset($parameters['Cobrador'])){
            ...
            {
                $objP = VNegociosPagadores::find($parameters['Cobrador'][0]["id"]);
                $parametersP['id'] =$parameters['Cobrador'][0]["id"];
                $parametersP['idNegocio'] =$negocio->id;
                $parametersP['CURP'] =$parameters['Cobrador'][0]["CURP"];
                $parametersP['Nombre'] =$parameters['Cobrador'][0]["Nombre"];
                $parametersP['Paterno'] =$parameters['Cobrador'][0]["Paterno"];
                $parametersP['Materno'] =$parameters['Cobrador'][0]["Materno"];
                $parametersP['idStatus'] =$parameters['Cobrador'][0]["idStatus"];
                $parameters['UserUpdated'] = $user->id;
        ...
        if(isset($parameters['QuiereTransferencia'])){
            if($parameters['QuiereTransferencia'] === true || ($parameters['QuiereTransferencia'])==1){
        ...
        if(isset($parameters['Giros'])){
            ...
            $giros = VNegociosGiros::where('idNegocio','=',$negocio->id)->get();

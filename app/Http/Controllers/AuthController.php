...
class AuthController extends Controller
{
	...
	function getUsersApp(Request $request){
        ...
        try {
            ...
            ->select(
				'U.id', 'U.email', 'U.Nombre', 'U.Paterno', 'U.Materno', 'U.idTipoUser', 'UT.TipoUser', 'U.TelCasa', 
				'U.TelCelular', 'U.Correo', 'U.Calle', 'U.NumExt', 'U.NumInt', 'U.Colonia', 'U.CP', 
				'U.idMunicipio', 'M.Municipio',
				 'U.idLocalidad', 
				'U.defaultPage', 
				'U.DevideID', 'U.DeviceOS', 'U.Token', 'U.Foto64', 'U.created_at', 'U.updated_at', 'U.idStatus'
			 )
	...
	public  function  login(Request  $request) {
		...
		if (!$jwt_token = JWTAuth::attempt($input)) {
			...
			$response =  ['success'=>true,'results'=>false,
			'errors'=>$errors, 'message' =>'Usuario y/o contraseña incorrectas.'];
		...
		$usertipo = DB::table('cat_usertipo')->where('id','=',$user->idTipoUser)->first();
		$user->idTipoUser = $usertipo;
		
	
		$usermunicipios = DB::table('cat_municipio')->where('id','=',$user->idMunicipio)->first();
		$user->idMunicipio = $usermunicipios;

		$userlocalidad = DB::table('cat_localidades')->where('mapa','=',$user->idLocalidad)->first();
		...
		if($user->idStatus == 1){
			...
			->where('users_menus.idUser','=',$user->id)->get();
			...
			->where('users_region.idUser','=',$user->id)->get();
			...
			'token'=>$jwt_token];
				...
				$response =  ['success'=>true,'results'=>true,
			'token'=>$jwt_token, 'user' =>$user, 'menu'=>$res,
			...
			$response =  ['success'=>true,'results'=>false,
			'errors'=>$errors, 'message' =>'Cuenta pendiente de autorizar.'];
		}
		else{
			$errors = [
				"Clave"=>"02"
			...
			$response =  ['success'=>true,'results'=>false,
			'errors'=>$errors, 'message' =>'Cuenta bloqueada.'];
		}
	...
	public function updateUserPassword(Request $request)
    {
        ...
        if(Hash::check($old, $usuario->password)){
			...
			$response =  ['success'=>true,'results'=>false,
			'errors'=>$errors, 'message' =>'La contraseña no es correcta, debe ingresar la contraseña actual antes de cambiarla.'];
            return  response()->json($response,200);
        }
	...
	function AgregarMultiples(){
		...
		if(count($usuarios_nuevos)  === 0){
			$response =  ['success'=>true,'results'=>false,
			'message' =>'No hay datos en la tabla _personasagregar, para generar nuevos usuarios. '];
        	return  response()->json($response,200);
		}
		$contador = 0;
		...
		foreach($usuarios_nuevos as $request){
			...
			$user_buscado = User::where('email','=',$request->Celular)->first();
			if($user_buscado){

...
class ZisController extends Controller
{
    public function load(Request $request)
    {
        ...
        try{
            ...
            if($columns < 1)
            {
                return ['success' => true, 'results' => false, 'message'=>'La primer fila no contiene encabezados','error' =>""];
            }

            if($columns > $maxField + 4)
            {
                return ['success' => true, 'results' => false, 'message'=>'El archivo contiene más columnas que la tabla destino. Revise el archivo y vuelva a intentarlo.','error' =>""];                    
            }
            
            $flag = is_int($headers[0]);
            
            if($flag){
                return ['success' => true, 'results' => false, 'message'=>'La primera fila no contiene el formato correcto para los encabezados. Revise el archivo y vuelva a intentarlo.','error' =>""];
            }
            ...
            if(count($notFound) > 0){
                return ['success' => true, 'results' => false, 'message'=>'La primera fila debe contener los titulos --> '.implode(',', $notFound),'error' =>""];
            }
            ...
            for ($startRow =1; $startRow <= 65536; $startRow += $chunkSize) {
                ...
                $data_insert = collect($data)->map(function($row) use($minField, $requireds){
                    $count = 1;
                    $newRow = [];
                    ...
                    $newRow = array_filter($newRow,function($key){
            ...
            return ['success' => true,'results' => true, 'message'=>'archivo cargado con exito'];
        }
        catch (Exception $e){ 
            DB::rollBack();
           return ['success' => false,'results' => false, 'message'=>'Error','error' => $e ];
        }
    }
    ...
    public function getZis(){
        ...
        $data = DB::table('vales_zis')->where('actualizado','!=','1')->get();
            ...
            try{
                ...
                {   
                    ...
                    $array = array_map(function($x){
                        return[strval($x['n_0']),'https://sdsh.guanajuato.gob.mx/apis/datosapizis?lat=' . strval($x['lat']) . '&lon=' . strval($x['lon'])];
                    },$chunk);
        ...
        return response()->json(['success'=>true,'results'=>true,'message'=>'Se guardaron las zis con exito','inicio'=>$inicio,'fin'=>date('Y-m-d H:i:s')]);
    }

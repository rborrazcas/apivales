...
class ReportesController extends Controller
{
    function getAvanceRemesas(Request $request){
		...
		if ($v->fails()){
            $response =  ['success'=>false,'results'=>false, 'errors'=>$v->errors(),'data'=>[]];
        ...
        $catRemesas = DB::table('vales_remesas')
            ...
            ->orderBy('Fecha', 'desc')
            ->get()->pluck('Remesa')->toArray();

            //dd($catRemesas);

        try{
            $select = "G.Remesa, G.Region, G.Municipio, G.Responsable, G.Total, A.Avance, (G.Total -if(A.Avance is null, 0, A.Avance)) as Restan";
            
            $table1 = "(select Remesa,  Region, concat_ws('-',Remesa, Municipio, Responsable) Clave, Municipio, Responsable, Total from vales_grupos_totales  order by Region, Municipio, Responsable ) as G";
            
            $table2 = "(select concat_ws('-',Remesa, Municipio, Articulador) Clave, Municipio, Articulador, count(distinct idSolicitud) as Avance from vales_solicitudes  group by Remesa, Municipio, Articulador) as A ";
            ...
            return response()->json(['success'=>true,'results'=>true,'total'=>$res->count(),'data'=>$res]);
    ...
    function getMisRemesas(Request $request){
        ...
        try{
            $select = "R.Clave, R.Region, R.Municipio, R.Remesa, R.Total, if(E.Entregados is null, 0, E.Entregados) as Entregados, R.Responsable";
            
            $table1 = "(SELECT concat_ws('-',V.idMunicipio,  V.Remesa, UO.id) Clave, V.idMunicipio, M.SubRegion AS Region, M.Nombre AS Municipio, V.Remesa, count(V.id) as Total, concat_ws(' ',UO.Nombre, UO.Paterno, UO.Materno) Responsable FROM vales AS V JOIN et_cat_municipio AS M ON V.idMunicipio = M.Id JOIN users AS UO ON V.UserOwned = UO.id WHERE V.UserOwned=".$user->id." and V.idStatus=5 GROUP BY V.Remesa, V.idMunicipio, UO.id ORDER BY V.Remesa ASC, M.Nombre ASC) R";
            
            $table2 = "(SELECT V.idMunicipio, M.SubRegion AS Region, M.Nombre AS Municipio, V.Remesa, count(V.id) as Entregados, concat_ws(' ',UO.Nombre, UO.Paterno, UO.Materno) Responsable FROM vales AS V JOIN et_cat_municipio AS M ON V.idMunicipio = M.Id JOIN users AS UO ON V.UserOwned = UO.id WHERE V.UserOwned=".$user->id." and V.idStatus=5 and V.isEntregado=1 GROUP BY V.Remesa, V.idMunicipio, UO.id ORDER BY V.Remesa ASC, M.Nombre ASC) E";

            $res = DB::table(DB::raw($table1))
            ->select(DB::raw($select))
            ->leftJoin(DB::raw($table2),'R.idMunicipio', '=', 'E.idMunicipio')
    ...
    function getMiResumenVales(Request $request){
        ...
        try{
            ...
            return response()->json(['success'=>true,'results'=>true,'total'=>$resTotal[0]->Total,'data'=>$res]);
    ...
    function getMisRemesasTotales(Request $request){
        ...
        try{
            ...
            $select = "V.Remesa, V.Municipio, V.Total, V.UserOwned, if(NE.NoEntregado is null, 0, NE.NoEntregado) NoEntregado, if(E.Entregado is null, 0, E.Entregado) Entregado";
            $table1 = "(SELECT V.Remesa, M.Id as idMunicipio,  M.Nombre as Municipio, V.UserOwned, count(V.isEntregado) Total FROM vales V inner join et_cat_municipio M on (V.idMunicipio = M.Id) WHERE V.Remesa is not null group by V.UserOwned, V.Remesa, M.Id) V";
            $table2 = "(SELECT Remesa, idMunicipio, UserOwned, count(isEntregado) NoEntregado FROM vales WHERE Remesa is not null  and isEntregado=0 group by UserOwned, Remesa, idMunicipio) NE";
            $table3= "(SELECT Remesa, idMunicipio, UserOwned, count(isEntregado) Entregado FROM vales WHERE Remesa is not null  and isEntregado=1 group by UserOwned, Remesa, idMunicipio) E";
            ...
            $data =$res->orderBy('V.UserOwned','ASC')
            ...
            return response()->json(['success'=>true,'results'=>true,'total'=>$res->count(),'data'=>$data]);
    ...
    function getReporteNominaValesDetalle(Request $request){
        ...
        if(!$parameters["UserOwned"]){
            return response ()->json(['success'=>true,'results'=>false,'data'=>[],'message'=>'No se encontraron resultados del Articulador.']);
        }

        $res = DB::table('vales as N')
        ->select('N.id', 'N.idIncidencia', 'VSX.Incidencia',  'N.Remesa', DB::raw('YEAR(N.FechaSolicitud) as Ejercicio'),
            'M.SubRegion AS Region', DB::raw("LPAD(HEX(N.id),6,0) AS ClaveUnica"), 'N.CURP',
            DB::raw("concat_ws(' ',N.Nombre, N.Paterno, N.Materno) as NombreCompleto"),'N.Sexo',  
            DB::raw("concat_ws(' ',N.Calle, if(N.NumExt is null, ' ', concat('NumExt ',N.NumExt)), if(N.NumInt is null, ' ', concat('Int ',N.NumInt))) AS Direccion"),
             'N.Colonia', 'N.CP', 'M.Nombre AS Municipio', 'L.Nombre AS Localidad', 'VS.SerieInicial', 'VS.SerieFinal', 'N.isEntregado', 'N.entrega_at'
        )
        ->leftJoin('vales_incidencias as VSX','VSX.id','=','N.idIncidencia')
        ...
        ->where(DB::raw('YEAR(N.FechaSolicitud)'),'=',$parameters['Ejercicio']);



        if(isset($parameters["UserOwned"])) $res->where('N.UserOwned','=',$parameters["UserOwned"]);
        if(isset($parameters["idMunicipio"])) $res->where('N.idMunicipio','=',$parameters["idMunicipio"]);
        if(isset($parameters["Remesa"])) $res->where('N.Remesa','=',$parameters["Remesa"]);
        if(isset($parameters["isEntregado"])) $res->where('N.isEntregado','=',$parameters["isEntregado"]);
        if(isset($parameters["isIncidencia"])) $res->orWhere('N.idIncidencia','!=',1);


        $data = $res->orderBy('N.Remesa','asc')->orderBy('M.Nombre','asc')->orderBy('N.Colonia','asc')->orderBy('N.Nombre','asc')->orderBy('N.Paterno','asc')->get();
    ...
    function getCatGrupos(Request $request){
        ...
        try{
            ...
            ->get()->pluck('Municipio')->toArray();

            $catResponsable = DB::table('vales_grupos_totales')
            ->select('Responsable')
            ->groupBy('Responsable')
            ->orderBy('Responsable', 'ASC')
            ->get()->pluck('Responsable')->toArray();

            $data = ["Remesas"=>$catRemesas, "Municipios"=>$catMunicipio, "Responsables"=>$catResponsable];

            return response()->json(['success'=>true,'results'=>true,'total'=>count($data),'data'=>$data]);
    ...
    function getRemesasGruposAvance(Request $request){
        ...
        try{
            $select = "G.Remesa, G.Region, G.idMunicipio, G.Municipio, G.UserOwned, G.Responsable, G.Total, V.Avance, if(G.Total=V.Avance, 'Completado', format(100 *(if(V.Avance is null, 0, V.Avance)/G.Total),2)) as Estatus";
            
            $table1 = "(select Region, idMunicipio, Municipio, UserOwned, Responsable, Total, Remesa from vales_grupos_totales) G";
            
            $table2 = "(Select V.idMunicipio, V.UserOwned, V.Remesa, count(V.id) Avance from vales V inner join  vales_solicitudes VS on (VS.idSolicitud = V.id) group by V.idMunicipio, V.UserOwned, V.Remesa) V";
            ...
            {
                $res->where('G.Municipio', 'like', '%'.$parameters["Remesa"].'%');
            }

            if(isset($parameters["Responsable"]))
            {
                $res->where('G.Responsable', 'like', '%'.$parameters["Responsable"].'%');
            }
    ...
    public function getReporteGrupos(Request $request){
        ...
        return response()->download($file,$data2->MunicipioGrupo.'_'.$data2->NombreGrupo.'.xlsx');
    }   
    ...
    public function getReporteComercios(Request $request){
        ...
        ->select(
            'M.SubRegion AS Region', 'N.id', 'N.RFC', 'N.NombreEmpresa',
            DB::raw("concat_ws(' ',N.Nombre, N.Paterno, N.Materno) AS NombreContacto"),
            'N.TelNegocio', 'N.TelCasa', 'N.Celular', 'N.Correo', 'N.FechaInscripcion', 'N.HorarioAtencion',  'T.Tipo AS Categoria', 'N.Banco', 'N.CLABE', 'N.Calle', 'N.NumExt', 'N.NumInt', 'N.Colonia', 'N.CP', 'M.Nombre AS Municipio', 'N.Latitude', 'N.Longitude', 'E.Estatus'
        )
        ...
        $data = $res->orderBy('M.Nombre','asc')->orderBy('N.Colonia','asc')->orderBy('N.NombreEmpresa','asc')->get();
        //$data2 = $resGrupo->first();
        ...
        $reader = IOFactory::createReader('Xlsx');
        $spreadsheet = $reader->load(public_path() .'/archivos/formatoComercioVales.xlsx');
        $sheet = $spreadsheet->getActiveSheet();
        $largo = count($res);
        $impresion = $largo+17;
        ...
        */
        ...
        return response()->download($file,'ComerciosValesGrandeza'.date('Y-m-d').'.xlsx');
    } 
    ...
    public function downloadReporteComercios(Request $request){
        ...
        $res = DB::table('v_negocios as N')
        ->select(
            'M.SubRegion AS Region', 'N.id',
            DB::raw("LPAD(HEX(N.id),6,0) Folio"),
            'N.RFC', 'N.NombreEmpresa',
            DB::raw("concat_ws(' ',N.Nombre, N.Paterno, N.Materno) AS NombreContacto"),
            'N.TelNegocio', 'N.TelCasa', 'N.Celular', 'N.Correo', 'N.FechaInscripcion', 'N.HorarioAtencion',  'NT.Tipo AS Categoria', 'N.Banco', 'N.CLABE', 'N.Calle', 'N.NumExt', 'N.NumInt', 'N.Colonia', 'N.CP', 'M.Nombre AS Municipio', 'N.Latitude', 'N.Longitude', 'E.Estatus',
            'N.updated_at', DB::raw("concat_ws(' ',U.Nombre, U.Paterno, U.Materno) AS Actualizo")
        )
        ...
        if($filtro_usuario){
            ...
            if($intervalo->h===0){//Si es 0 es porque no ha pasado una hora.
                ...
                if(isset($parameters['Folio'])){
                    ...
                    $res->where(DB::raw('LPAD(HEX(N.id),6,0)'),'like','%'.$valor_id.'%');
                }
                            ...
                            if($flag==0){
                                switch ($parameters['filtered'][$i]['id']) {
                                    case "id":
                                        if(is_array ($parameters['filtered'][$i]['value'])){
                                            $res->whereIn('N.id',$parameters['filtered'][$i]['value']);
                                        }else{
                                            $res->where('N.id','=',$parameters['filtered'][$i]['value']);
                                        }    
                                    break;
                                    case 'ClaveUnica':
                                        $res->where(DB::raw('LPAD(HEX(N.id),6,0)'),'=',$parameters['filtered'][$i]['value']);   
                                    break;
                                    case 'RFC':
                                    case 'NombreEmpresa':
                                    case 'TelNegocio':
                                    ...
                                    case 'HorarioAtencion':
                                        $res->where('N.'.$parameters['filtered'][$i]['id'],'LIKE','%'.$parameters['filtered'][$i]['value'].'%');   
                                    break;
                                    case 'Refrendo2021':
                                        $res->where('N.'.$parameters['filtered'][$i]['id'],'=',$parameters['filtered'][$i]['value']);   
                                    break;
                                    case 'Contacto':
                                        $contacto_buscar = $parameters['filtered'][$i]['value'];
                                        $contacto_buscar = str_replace(" ","",$contacto_buscar);
                                        ...
                                        ,'like',"%".$contacto_buscar."%");   
                                    break;
                                    case 'idMunicipio':
                                        if(is_array ($parameters['filtered'][$i]['value'])){
                                            $res->whereIn('N.idMunicipio',$parameters['filtered'][$i]['value']);
                                        }else{
                                            $res->where('N.idMunicipio','=',$parameters['filtered'][$i]['value']);
                                        }    
                                    break;
                                    case 'Municipio':
                                        $res->where('M.Nombre','LIKE','%'.$parameters['filtered'][$i]['value'].'%');   
                                    break;
                                    case 'Region':
                                        if(is_array ($parameters['filtered'][$i]['value'])){
                                            $res->whereIn('M.SubRegion',$parameters['filtered'][$i]['value']);
                                        }else{
                                    ...
                                    case 'idTipoNegocio':
                                        if(is_array ($parameters['filtered'][$i]['value'])){
                                            $res->whereIn('N.idTipoNegocio',$parameters['filtered'][$i]['value']);
                                        }else{
                                            $res->where('N.idTipoNegocio','=',$parameters['filtered'][$i]['value']);
                                        }    
                                    break;
                                    case 'TipoNegocio':
                                    ...
                                    case 'Estatus':
                                        if(is_array ($parameters['filtered'][$i]['value'])){
                                            $res->whereIn('N.idStatus',$parameters['filtered'][$i]['value']);
                                        }else{
                                            $res->where('N.idStatus','=',$parameters['filtered'][$i]['value']);
                                        }  
                                    break;
    
                                    default:
                                        $res->where('XXX','LIKE','%'.$parameters['filtered'][$i]['value'].'%');   
                                    break;
                                ...
                                if($parameters['tipo']=='and'){
                                    switch ($parameters['filtered'][$i]['id']) {
                                        case 'id':
                                            if(is_array ($parameters['filtered'][$i]['value'])){
                                                $res->whereIn('id',$parameters['filtered'][$i]['value']);
                                            }else{
                                                $res->where('id','=',$parameters['filtered'][$i]['value']);
                                            }    
                                        break;
                                        case 'RFC':
                                        case 'NombreEmpresa':
                                    ...
                                    switch ($parameters['filtered'][$i]['id']) {
                                        case 'id':
                                            if(is_array ($parameters['filtered'][$i]['value'])){
                                                $res->orWhereIn('id',$parameters['filtered'][$i]['value']);
                                            }else{
                                                $res->orWhere('id','=',$parameters['filtered'][$i]['value']);
                                            }    
                                        break;
                                        case 'RFC':
                                        case 'NombreEmpresa':
                                        ...
                                        case 'FechaInscripcion':
                                        case 'HorarioAtencion':
                                            $res->orWhere('N.'.$parameters['filtered'][$i]['id'],'LIKE','%'.$parameters['filtered'][$i]['value'].'%');   
                                        break;
                                        case 'Refrendo2021':
                                            $res->orWhere('N.'.$parameters['filtered'][$i]['id'],'=',$parameters['filtered'][$i]['value']);   
                                        break;
                                        case 'Contacto':
                                            $contacto_buscar = $parameters['filtered'][$i]['value'];
                                            $contacto_buscar = str_replace(" ","",$contacto_buscar);
                                        ...
                                        case 'idMunicipio':
                                            if(is_array ($parameters['filtered'][$i]['value'])){
                                                $res->orWhereIn('N.idMunicipio',$parameters['filtered'][$i]['value']);
                                            }else{
                                                $res->orWhere('N.idMunicipio','=',$parameters['filtered'][$i]['value']);
                                            }    
                                        break;
                                        case 'Municipio':
                                            $res->orWhere('M.Nombre','LIKE','%'.$parameters['filtered'][$i]['value'].'%');   
                                        break;
                                        case 'Region':
                                            if(is_array ($parameters['filtered'][$i]['value'])){
                                                $res->orWhereIn('M.SubRegion',$parameters['filtered'][$i]['value']);
                                            }else{
                                        ...
                                        case 'idTipoNegocio':
                                            if(is_array ($parameters['filtered'][$i]['value'])){
                                                $res->orWhereIn('idTipoNegocio',$parameters['filtered'][$i]['value']);
                                            }else{
                                                $res->orWhere('idTipoNegocio','=',$parameters['filtered'][$i]['value']);
                                            }    
                                        break;
                                        case 'TipoNegocio':
                                            $res->orWhere('NT.Tipo','LIKE','%'.$parameters['filtered'][$i]['value'].'%');   
                                        break;
                                        
                                        case 'Estatus':
                                            if(is_array ($parameters['filtered'][$i]['value'])){
                                                $res->orWhereIn('N.idStatus',$parameters['filtered'][$i]['value']);
                                            }else{
                                                $res->orWhere('N.idStatus','=',$parameters['filtered'][$i]['value']);
                                            }  
                                        break;
                                        default:
                                            $res->where('XXX','LIKE','%'.$parameters['filtered'][$i]['value'].'%');   
    ...
    public function getReportesolicitudValesOldOriginal(Request $request){
        ...
        $res = DB::table('vales as N')
        ->select(
            'M.SubRegion AS Region', DB::raw("LPAD(HEX(N.id),6,0) AS ClaveUnica"), 'N.FechaSolicitud', 'N.CURP',
           'N.Nombre', 'N.Paterno', 'N.Materno', DB::raw("IF (N.Sexo = 'M', 'H', 'M')"), 'N.FechaNacimiento', 'N.Calle', 'N.NumExt', 'N.NumInt',
             'N.Colonia', 'N.CP', 'M.Nombre AS Municipio', 'L.Nombre AS Localidad',
            'N.TelFijo', 'N.TelCelular', 'N.Compania', 'N.TelRecados', 'N.CorreoElectronico', 'E.Estatus'
            , DB::raw("concat_ws(' ',UC.Nombre, UC.Paterno, UC.Materno) as UserInfoCapturo")
            , DB::raw("concat_ws(' ',UOC.Nombre, UOC.Paterno, UOC.Materno) as UserInfoOwned")
        )
        ->leftJoin('et_cat_municipio as M','N.idMunicipio','=','M.Id')
        ->leftJoin('et_cat_localidad as L','N.idLocalidad','=','L.Id')
        ...
        $data = $res->orderBy('M.Nombre','asc')->orderBy('N.Colonia','asc')->orderBy('N.Nombre','asc')->orderBy('N.Paterno','asc')->get();
        //$data2 = $resGrupo->first();
       ...
       // dd($data);
        ...
        if(count($data)==0)
        {
            ...
            $reader = IOFactory::createReader('Xlsx');
            $spreadsheet = $reader->load(public_path() .'/archivos/formatoReporteSolicitudVales.xlsx');
            $writer = new Xlsx($spreadsheet);
            $writer->save('archivos/'.$user->email.'reporteComercioVales.xlsx');
            $file = public_path() .'/archivos/'.$user->email.'reporteComercioVales.xlsx';

            return response()->download($file,'SolicitudesValesGrandeza'.date('Y-m-d').'.xlsx');
        ...
        return response()->download($file,$user->email.'SolicitudesValesGrandeza'.date('Y-m-d').'.xlsx');
    }
    ...
    public function getReportesolicitudVales(Request $request){
        ...
        $res = DB::table('vales')
            ->select(
                ...
                'vales.OtrosIngresos',
                DB::raw("(vales.IngresoPercibido + vales.OtrosIngresos) as TotalIngresos"),
                ...
                DB::raw("concat_ws(UD.Nombre, UD.Paterno, UD.Materno) as UserDocumentacion"),
                'vales.Remesa',
                'vales_status.Estatus',
                DB::raw("concat_ws(' ',users.Nombre, users.Paterno, users.Materno) as UserInfoCapturo"),
                DB::raw("concat_ws(' ',usersC.Nombre, usersC.Paterno, usersC.Materno) as UserInfoOwned") 
            )
            ->leftJoin('vales_incidencias','vales_incidencias.id','=','vales.idIncidencia')
        ...
        $filtro_usuario=VNegociosFiltros::where('idUser','=',$user->id)->where('api','=','getValesV2')->first();
        if($filtro_usuario){
            $hoy = date("Y-m-d H:i:s");    
            $intervalo = $filtro_usuario->updated_at->diff($hoy);
            if($intervalo->h===0){//Si es 0 es porque no ha pasado una hora.
                ...
                {
                    $res->where(DB::raw('YEAR(vales.FechaSolicitud)'),'=',date("Y"));
                }
            ...
            if(isset($parameters['Duplicados'])){
                if($parameters['Duplicados']==1){
                    if($flag_ejercicio){
                        $res->whereRaw(DB::raw('CURP  in (Select CURP from vales where  YEAR(vales.FechaSolicitud) = '.$parameters['Ejercicio'].'  group by CURP HAVING count(CURP)>1)'));
                    }
                    else{
                        $res->whereRaw(DB::raw('CURP  in (Select CURP from vales group by CURP HAVING count(CURP)>1)'));
                    }
            ...
            if(isset($parameters['Ejercicio']) && !is_null($parameters['Ejercicio'])){
                ...
                $res->where(DB::raw('YEAR(vales.FechaSolicitud)'),'=', $valor_id);
            }else
            {
                $res->where(DB::raw('YEAR(vales.FechaSolicitud)'),'=', date("Y"));
            }

            if(isset($parameters['idMunicipio']) && !is_null($parameters['idMunicipio']) ){
            ...
            if(isset($parameters['Colonia']) && !is_null($parameters['Colonia']) ){
            ...
            if(isset($parameters['idStatus']) && !is_null($parameters['idStatus']) ){
            ...
            if(isset($parameters['UserOwned']) && !is_null($parameters['UserOwned']) ){
            ...
            if(isset($parameters['Remesa']) && !is_null($parameters['Remesa']) ){
            ...
            if(isset($parameters['filtered'])){
                for($i=0;$i<count($parameters['filtered']);$i++){
                    if($parameters['filtered'][$i]['id']==='FechaCapturaFin' && $parameters['filtered'][$i]['value']!=='' )
                    {
                        $FiltroCreated_atFin= $parameters['filtered'][$i]['value'];
                        $banFechaFin=2;
                    }
                    else if($parameters['filtered'][$i]['id']=='vales.created_at' && $parameters['filtered'][$i]['value']!=='')
                    {
                        $FiltroCreated_at= $parameters['filtered'][$i]['value'];
                        $banFechaInicio=1;$banFechaFin=1;
                    }
                    else{
                        if($flag==0){
                                ...
                                if(strcmp($parameters['filtered'][$i]['id'], 'vales.UserCreated') === 0 || strcmp($parameters['filtered'][$i]['id'], 'vales.UserUpdated') === 0 || strcmp($parameters['filtered'][$i]['id'], 'vales.UserOwned') === 0){
                                    ...
                                    else{ $res->where($parameters['filtered'][$i]['id'],'LIKE','%'.$parameters['filtered'][$i]['value'].'%');}
            ...
            {
                $res->whereRaw("(DATE(vales.created_at) BETWEEN '".$FiltroCreated_at."' AND '".$FiltroCreated_atFin."')");
                
            }
            else if($banFechaFin == 1 && $banFechaInicio == 1)
            {
                $res->whereRaw("(DATE(vales.created_at) = '".$FiltroCreated_at."')");
            }
            ...
            if(isset($parameters['NombreCompleto']) && !is_null($parameters['NombreCompleto'])){
                $filtro_recibido = $parameters['NombreCompleto'];
                $filtro_recibido = str_replace(" ","",$filtro_recibido);
                $res->where(
                    DB::raw("
            ...
            if(isset($parameters['NombreOwner']) && !is_null($parameters['NombreOwner'])){
            ...
            if(isset($parameters['NombreCreated']) && !is_null($parameters['NombreCreated'])){
        ...
        $data = $res->orderBy('vales.Nombre','asc')->orderBy('vales.Colonia','asc')->orderBy('vales.Nombre','asc')->orderBy('vales.Paterno','asc')->get();
        //$data2 = $resGrupo->first();
        ...
        if(count($data)==0)
        {
            //return response()->json(['success'=>false,'results'=>false,'message'=>$res->toSql()]);
            $reader = IOFactory::createReader('Xlsx');
            $spreadsheet = $reader->load(public_path() .'/archivos/formatoReporteSolicitudValesV3.xlsx');
            $writer = new Xlsx($spreadsheet);
            $writer->save('archivos/'.$user->email.'reporteComercioVales.xlsx');
            $file = public_path() .'/archivos/'.$user->email.'reporteComercioVales.xlsx';
        ...
        $reader = IOFactory::createReader('Xlsx');
        $spreadsheet = $reader->load(public_path() .'/archivos/formatoReporteSolicitudValesV4.xlsx');
        $sheet = $spreadsheet->getActiveSheet();
        $largo = count($res);
        $impresion = $largo+10;
        ...
        //         $veces++;
        //         //dd($largo);
                $sheet->setBreak('B'.($lb+10), \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::BREAK_ROW);
             }
        ...
        //guardamos el excel creado y luego lo obtenemos en $file para poder descargarlo
        $writer = new Xlsx($spreadsheet);
        $writer->save('archivos/'.$user->email.'SolicitudesValesGrandeza.xlsx');
        $file = public_path() .'/archivos/'.$user->email.'SolicitudesValesGrandeza.xlsx';

        return response()->download($file,$user->email.'SolicitudesValesGrandeza'.date('Y-m-d H:i:s').'.xlsx');
    }
    ...
    public function getPadronPotencial(Request $request){
        ...
        $res = DB::table('vales')
            ->select(
                'vales.Nombre', 'vales.Paterno', 'vales.Materno',
                'vales.FechaNacimiento', 
                ...
                'et_cat_municipio.Nombre AS Municipio',
                DB::raw("concat('11', lpad(et_cat_localidad.IdMunicipio,3,0), lpad(sedeshu.et_cat_localidad.Numero, 4, 0)) as NumeroLocalidad"),
                'et_cat_localidad.Nombre AS Localidad',
                'vales.Colonia',
                DB::raw("' ' as Manzana"),
                'vales.Calle', 'vales.NumExt', 'vales.NumInt', 'vales.CP',
            ...
            ->leftJoin('cat_edo_curp as EC',DB::raw("SUBSTRING(vales.CURP, 12, 2)"),'=','EC.Clave')
            ->leftJoin('et_cat_municipio','et_cat_municipio.Id','=','vales.idMunicipio')
            ->leftJoin('et_cat_localidad','et_cat_localidad.Id','=','vales.idLocalidad')
            ->leftJoin('vales_status','vales_status.id','=','idStatus')
            ->leftJoin('users','users.id','=','vales.UserCreated')
            ...
            ->whereRaw('vales.CURP not in (Select CURP from vales where YEAR(FechaSolicitud)='.date("Y").' and Remesa is not null)');
        ...
        if(count($data)==0)
        {
            //return response()->json(['success'=>false,'results'=>false,'message'=>$res->toSql()]);
            $reader = IOFactory::createReader('Xlsx');
            $spreadsheet = $reader->load(public_path() .'/archivos/PadronPotencial2021_Personas.xlsx');
            $writer = new Xlsx($spreadsheet);
            $writer->save('archivos/'.$user->email.'PadronPotencial2021_Personas.xlsx');
            $file = public_path() .'/archivos/'.$user->email.'PadronPotencial2021_Personas.xlsx';

            return response()->download($file,'PadronPotencial2021_Personas'.date('Y-m-d H:i:s').'.xlsx');
        ...
        //         $veces++;
        //         //dd($largo);
                $sheet->setBreak('A'.($lb+10), \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::BREAK_ROW);
             }
        ...
        return response()->download($file,$user->email.'PadronPotencial2021_Personas'.date('Y-m-d H:i:s').'.xlsx');
    }
    ...
    public function getReportesolicitudValesDesglosado(Request $request){
        ...
        $res = DB::table('vales')
            ->select(
                ...
                'vales.Ocupacion',
                //DB::raw("concat_ws(' ',vales.Calle, concat('Num. ', vales.NumExt), if(vales.NumInt is not null,concat('NumExt. ',vales.NumInt), ''), concat('Col. ',vales.Colonia)) as Direccion"),
                DB::raw('CASE WHEN vales.Calle is null then "S/C" else vales.Calle end as Calle'),
                DB::raw('CASE WHEN vales.NumExt is null then "S/N" else concat("Num. ",vales.NumExt) end as NumExt'),
                DB::raw('CASE WHEN vales.NumInt is null then "S/N" else concat("Num. ",vales.NumInt) end as NumInt'),
                DB::raw('CASE WHEN vales.Colonia is null then "S/C" else concat("Col. ",vales.Colonia) end as Colonia'),
        ...
        if($filtro_usuario){
            ...
            if(isset($parameters['filtered'])){
                ...
                for($i=0;$i<count($parameters['filtered']);$i++){
                    ...
                    if($parameters['filtered'][$i]['id']=='vales.created_at' && $parameters['filtered'][$i]['value']!=='')
                    {
                        $FiltroCreated_at= $parameters['filtered'][$i]['value'];
                        $banFechaInicio=1;$banFechaFin=1;
                    }
        ...
        $data = $res->orderBy('vales.Nombre','asc')
        ...
        if(count($data)==0)
        {
            //return response()->json(['success'=>false,'results'=>false,'message'=>$res->toSql()]);
            $reader = IOFactory::createReader('Xlsx');
            $spreadsheet = $reader->load(public_path() .'/archivos/formatoReporteSolicitudValesDesglosado.xlsx');
            $writer = new Xlsx($spreadsheet);
            $writer->save('archivos/'.$user->email.'formatoReporteSolicitudValesDesglosado.xlsx');
            $file = public_path() .'/archivos/'.$user->email.'formatoReporteSolicitudValesDesglosado.xlsx';

            return response()->download($file,'formatoReporteSolicitudValesDesglosado'.date('Y-m-d').'.xlsx');
        ...
        return response()->download($file,$user->email.'formatoReporteSolicitudValesDesglosado'.date('Y-m-d').'.xlsx');
    }
    ...
    public function getRepValesExpedientes(Request $request){
        ...
        $res = DB::table('vales as V')
            ...
            ->whereNull('V.Remesa');
                ...
                */
                ...
                'V.FechaSolicitud',
                'V.CURP', 
                DB::raw("concat_ws(' ',V.Nombre, V.Paterno, V.Materno) as NombreCompleto"),
                'V.Sexo', 
                'V.FechaNacimiento', 
                DB::raw("concat_ws(' ',V.Calle, concat('Num. ', V.NumExt), if(V.NumInt is not null,concat('NumExt. ',V.NumInt), ''), concat('Col. ',V.Colonia)) as Direccion"),
                'V.CP', 
                'M.Nombre AS Municipio',
                'et_cat_localidad.Nombre AS Localidad',
                DB::raw("CASE WHEN V.TelFijo is null THEN 'S/T' ELSE V.TelFijo END as TelFijo"),
                DB::raw("CASE WHEN V.TelCelular is null THEN 'S/T' ELSE V.TelCelular END as TelCelular"),
                DB::raw("CASE WHEN V.Compania is null THEN 'S/C' ELSE V.Compania END as Compania"),
                DB::raw("CASE WHEN V.TelRecados is null THEN 'S/T' ELSE V.TelRecados END as TelRecados"),
                DB::raw("CASE WHEN V.CorreoElectronico is null THEN 'S/C' ELSE V.CorreoElectronico END as CorreoElectronico"),
                'V.FechaDocumentacion',
                DB::raw("concat_ws(' ',U.Nombre, U.Paterno, U.Materno) as Recepciono"),
                DB::raw("concat_ws(' ',users.Nombre, users.Paterno, users.Materno) as UserInfoCapturo"),
                DB::raw("concat_ws(' ',R.Nombre, R.Paterno, R.Materno) as UserInfoOwned"),
                DB::raw("concat_ws(' ',V.IngresoPercibido) as IngresoPercibido"),
        ...
        $filtro_usuario=VNegociosFiltros::where('idUser','=',$user->id)->where('api','=','getValesDocumentacion')->first();
        ...
        if($filtro_usuario){
            ...
            if(isset($parameters['Articulador'])){
                ...
                if(is_array ($parameters['Articulador'])){
                    ...
                    $res->where(function($q)use ($valor_id) {
                        $q->whereIn('V.UserCreated', $valor_id)
            ...
            $view_all = DB::table('users_menus')->where('idUser', $user->id)->where('idMenu',9)->first();
                ...
                for($i=0;$i<count($parameters['filtered']);$i++){
                    ...
                    if($flag==0){
                        if($parameters['filtered'][$i]['id'] &&  strpos($parameters['filtered'][$i]['id'], 'id') !== false){
                            ...
                            if(strcmp($parameters['filtered'][$i]['id'], 'V.UserCreated') === 0 || strcmp($parameters['filtered'][$i]['id'], 'V.UserUpdated') === 0 || strcmp($parameters['filtered'][$i]['id'], 'V.UserOwned') === 0){
                                $res->where($parameters['filtered'][$i]['id'],'=',$parameters['filtered'][$i]['value']);
                            }
                            else{
                                if(strpos($parameters['filtered'][$i]['id'], 'is') !== false){
                        ...
                        if($parameters['tipo']=='and'){
                            ...
                            if($parameters['filtered'][$i]['id'] && strpos($parameters['filtered'][$i]['id'], 'id') !== false){
                                ...
                                if(strcmp($parameters['filtered'][$i]['id'], 'V.UserCreated') === 0 || strcmp($parameters['filtered'][$i]['id'], 'V.UserUpdated') === 0){
                                    $res->orWhere($parameters['filtered'][$i]['id'],'=',$parameters['filtered'][$i]['value']);
                                }else{
                                    if(strpos($parameters['filtered'][$i]['id'], 'is') !== false){
       ...
       //dd($res->get());
        ...
        $data = $res->orderBy('Recepciono','asc')
        ...
        if(count($data)==0)
        {
            //return response()->json(['success'=>false,'results'=>false,'message'=>$res->toSql()]);
            $reader = IOFactory::createReader('Xlsx');
            $spreadsheet = $reader->load(public_path() .'/archivos/formatoReporteExpedientesRecibidos.xlsx');
            $writer = new Xlsx($spreadsheet);
            $writer->save('archivos/'.$user->email.'ReporteExpedientesRecibidos.xlsx');
            $file = public_path() .'/archivos/'.$user->email.'ReporteExpedientesRecibidos.xlsx';

            return response()->download($file,'ReporteExpedientesRecibidos'.date('Y-m-d H:i:s').'.xlsx');
        ...
        return response()->download($file,$user->email.'ReporteExpedientesRecibidos'.date('Y-m-d H:i:s').'.xlsx');
    }
    ...
    public function getReporteFoliosValidar(Request $request){
        ...
        $res_ant = DB::table('_folios_validar')
            ->select('Folio');

            $res_ant = $res_ant->offset($page)
            ->take($pageSize)
            ->get();
            ...
            foreach ($res_ant as $key => $value) {
                ...
                ->select(
                    ...
                    //DB::raw(''.$val.' as Folio'),
                    'SerieInicial', 'SerieFinal',
                    'CURP', DB::raw('CONCAT_WS(" " , Nombre,Paterno,Materno) as Nombre')
                    , DB::raw('CASE WHEN Municipio is null then "S/M" else Municipio end as Municipio'), 'Remesa'
                    )
                ->whereNull('Ejercicio')
...
//return $res;
            if(count($res)==0)
        {
            ...
            $reader = IOFactory::createReader('Xlsx');
            $spreadsheet = $reader->load(public_path() .'/archivos/validar_f.xlsx');
            $writer = new Xlsx($spreadsheet);
            $writer->save('archivos/'.$user->email.'validar_f.xlsx');
            $file = public_path() .'/archivos/'.$user->email.'validar_f.xlsx';

            return response()->download($file,'validar_f'.date('Y-m-d H:i:s').'.xlsx');
        ...
        return response()->download($file,$user->email.'validar_f'.date('Y-m-d H:i:s').'.xlsx');
    ...
    public function getReportesoVales(Request $request){
        ...
        $res = DB::table('vales')
        ->select(
            'et_cat_municipio.SubRegion AS Region', DB::raw("LPAD(HEX(vales.id),6,0) AS ClaveUnica"), 'vales.FechaSolicitud', 'vales.CURP',
           'vales.Nombre', 'vales.Paterno', 'vales.Materno','vales.Sexo', 'vales.FechaNacimiento', 'vales.Calle', 'vales.NumExt', 'vales.NumInt',
             'vales.Colonia', 'vales.CP', 'et_cat_municipio.Nombre AS Municipio', 'et_cat_localidad.Nombre AS Localidad',
            'vales.TelFijo', 'vales.TelCelular', 'vales.Compania', 'vales.TelRecados', 'vales.CorreoElectronico', 'vales_status.Estatus'
            , DB::raw("concat_ws(' ',users.Nombre, users.Paterno, users.Materno) as UserInfoCapturo")
            , DB::raw("concat_ws(' ',usersC.Nombre, usersC.Paterno, usersC.Materno) as UserInfoOwned")
        )
        ->leftJoin('et_cat_municipio','et_cat_municipio.Id','=','vales.idMunicipio')
        ...
        $data = $res->orderBy('et_cat_municipio.Nombre','asc')->orderBy('vales.Colonia','asc')->orderBy('vales.Nombre','asc')->orderBy('vales.Paterno','asc')->get();
        
        if(count($data)>=3000){
            return ['success'=>true,'results'=>false, 'total'=>count($data),'errors'=>'Limite de registros excedido','message'=>'El reporte contiene mas de 3,000 registros. Contacte al administrador.'];
        }
    ...
    public function getReportesoValesRegionales(Request $request){
        ...
        ->select(
           ...
           'vales.Nombre', 'vales.Paterno', 'vales.Materno','vales.Sexo', 'vales.FechaNacimiento', 'vales.Calle', 'vales.NumExt', 'vales.NumInt',
            ...
            'vales.TelFijo', 'vales.TelCelular', 'vales.Compania', 'vales.TelRecados', 'vales.CorreoElectronico', 'vales_status.Estatus'
            , DB::raw("concat_ws(' ',users.Nombre, users.Paterno, users.Materno) as UserInfoCapturo")
            , DB::raw("concat_ws(' ',UO.Nombre, UO.Paterno, UO.Materno) as UserInfoOwned")
        )
        /* ->leftJoin('et_cat_municipio','et_cat_municipio.Id','=','vales.idMunicipio')
        ->leftJoin('et_cat_localidad','et_cat_localidad.Id','=','vales.idLocalidad')
        ->leftJoin('vales_status','vales_status.id','=','idStatus')
        ...
        ->leftJoin('users_region as UR','UR.Region','=','et_cat_municipio.SubRegion')
        ...
        $user = auth()->user();
        $filtro_usuario=VNegociosFiltros::where('idUser','=',$user->id)->where('api','=','getValesRegion')->first();
        if($filtro_usuario){
            $hoy = date("Y-m-d H:i:s");    
            $intervalo = $filtro_usuario->updated_at->diff($hoy);
            if($intervalo->h===0){//Si es 0 es porque no ha pasado una hora.
    ...
    public function getReporteNominaVales(Request $request){
        ...
        if(!isset($request->idGrupo)){
            return response ()->json(['success'=>true,'results'=>false,'data'=>[],'message'=>'No se encontraron resultados del Grupo.']);
        }

        $resGpo = DB::table('vales_grupos as G')
        ->select('G.id', 'R.NumAcuerdo', 'R.Leyenda', 'R.FechaAcuerdo', 'G.UserOwned', 'G.idMunicipio','M.Nombre AS Municipio', 'G.Remesa',DB::raw("concat_ws(' ',UOC.Nombre, UOC.Paterno, UOC.Materno) as UserInfoOwned"))
        ...
        ->select(
            ...
            DB::raw("concat_ws(' ',N.Nombre, N.Paterno, N.Materno) as NombreCompleto"),'N.Sexo',  
            DB::raw("concat_ws(' ',N.Calle, if(N.NumExt is null, ' ', concat('NumExt ',N.NumExt)), if(N.NumInt is null, ' ', concat('Int ',N.NumInt))) AS Direccion"),
             'N.Colonia', 'N.CP', 'M.Nombre AS Municipio', 'L.Nombre AS Localidad', 'VS.SerieInicial', 'VS.SerieFinal'
        )
        ->leftJoin('et_cat_municipio as M','N.idMunicipio','=','M.Id')
        ->leftJoin('et_cat_localidad as L','N.idLocalidad','=','L.Id')
        ->Join('vales_solicitudes as VS','VS.idSolicitud','=','N.id')
    ...
    //     dd($data);
        ...
        if(count($data)==0)
        {
            ...
            $reader = IOFactory::createReader('Xlsx');
            $spreadsheet = $reader->load(public_path() .'/archivos/formatoReporteNominaValesv3.xlsx');
            ...
            $sheet = $spreadsheet->getActiveSheet();
             ...
             $sheet->setCellValue('A3', 'Aprobados mediante '.$resGpo->NumAcuerdo.' de fecha '.$resGpo->FechaAcuerdo);


            $writer = new Xlsx($spreadsheet);
            $writer->save('archivos/'.$resGpo->Remesa.'_'.$resGpo->idMunicipio.'_'.$resGpo->UserOwned.'_formatoNominaVales.xlsx');
            $file = public_path() .'/archivos/'.$resGpo->Remesa.'_'.$resGpo->idMunicipio.'_'.$resGpo->UserOwned.'_formatoNominaVales.xlsx';

            return response()->download($file,$resGpo->Remesa.'_'.$resGpo->idMunicipio.'_'.$resGpo->UserOwned.'_NominaValesGrandeza'.date('Y-m-d').'.xlsx');
        ...
        if($largo>25)
        {
            ...
            for($lb=20; $lb < $largo; $lb+=20)
            {
                $veces++;
                //dd($largo);
                $spreadsheet->getActiveSheet()->setBreak('A'.($lb+10), \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::BREAK_ROW);
            }
        ...
        $spreadsheet->getActiveSheet()->setBreak('A'.($impresion), \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::BREAK_ROW);
        ...
        $sheet->getStyle('C'.$ln.':O'.$ln)->getBorders()->getTop()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('C'.$ln.':O'.$ln)->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('C'.$ln)->getBorders()->getLeft()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('H'.$ln)->getBorders()->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('O'.$ln)->getBorders()->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        ...
        $sheet->setCellValue('F'.($ln+1), 'COORDINADOR DE CONTROL DE PAGOS');
        ...
        $sheet->getStyle('A'.$ln.':O'.$ln)->getBorders()->getTop()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('A'.$ln.':O'.$ln)->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('B'.$ln)->getBorders()->getTop()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('B'.$ln)->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('A'.$ln)->getBorders()->getLeft()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('B'.$ln)->getBorders()->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('E'.$ln)->getBorders()->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('G'.$ln)->getBorders()->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('H'.$ln)->getBorders()->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('K'.$ln)->getBorders()->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('N'.$ln)->getBorders()->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('O'.$ln)->getBorders()->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        ...
        $sheet->getStyle('A'.$ln.':O'.$ln)->getBorders()->getTop()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('A'.$lnf.':O'.$lnf)->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('A'.$ln.':A'.$lnf)->getBorders()->getLeft()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->getStyle('O'.$ln.':O'.$lnf)->getBorders()->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
       ...
       // dd($strRem);
        ...
        $writer->save('archivos/'.$strRem.'_'.$resGpo->idMunicipio.'_'.$resGpo->UserOwned.'_formatoNominaVales.xlsx');
        $file = public_path() .'/archivos/'.$strRem.'_'.$resGpo->idMunicipio.'_'.$resGpo->UserOwned.'_formatoNominaVales.xlsx';

        //dd('Se crearon los archivos');

        return response()->download($file,$strRem.'_'.$resGpo->idMunicipio.'_'.$resGpo->UserOwned.'_formatoNominaVales'.date('Y-m-d H:i:s').'.xlsx');
    }
    ...
    public function getSumaVoluntadesWord(Request $request){
        ...
        ->select('N.id',
            ...
            DB::raw("if(FechaInscripcion, date_format(FechaInscripcion,'%d'),' ') DD"),
            DB::raw("if(FechaInscripcion, date_format(FechaInscripcion,'%m'),' ') MM")
        )
        ->join('et_cat_municipio as M','N.idMunicipio','=','M.Id')
        ...
        $templateProcessor = new TemplateProcessor(public_path() .'/archivos/Suma_Voluntades_Comercio_ANEXO_2 04052020.docx');
        ...
        $templateProcessor->setImageValue('Barcode', array('path' => $d->getBarcodePNGPath($data->ClaveUnica, 'C128'), 'width' => 120, 'height' => 70, 'ratio' => false));
    ...
    public function getTarjeta(Request $request){
        ...
        ->select(
            DB::raw("concat_ws(' ', lpad(HEX(t.idGrupo),3,'0'),m.Nombre) as Grupo"),
            'a.FolioC','a.NombreC as Nombre','a.PaternoC as Paterno','a.MaternoC as Materno','m.Nombre as Municipio','l.Nombre as Localidad','t.Terminacion'
        )
        ...
        ->orderBy('mg.Nombre','asc')->orderBy('g.NombreGrupo','asc')->orderBy('NombreC','asc')->orderBy('PaternoC','asc')->orderBy('MaternoC','asc')
        ->first();
    ...
    public function getTarjetasByGrupo(Request $request){
        ...
        //borramos el zip..
        File::delete(public_path().'archivos/grupo'.$request->idGrupo.'.zip');
        ...
        for($i=0;$i<count($data);$i++){
            ...
            $templateProcessor = $phpWord->loadTemplate(public_path() .'/archivos/formatoWord.docx');
            ...
            $templateProcessor->saveAs('tarjetas/tarjeta'.$data[$i]->CURP.'.docx');
        }
            //Se utiliza la libreria de Zipper para generar el archivo .zip
        $files = glob(public_path('tarjetas/*'));
        Zipper::make(public_path('archivos/grupo'.$request->idGrupo.'.zip'))->add($files)->close();
        
        //Descargamos el zip
        return response()->download(public_path('archivos/grupo'.$request->idGrupo.'.zip'));
    }
    ...
    public function getVales(){
        ...
        $porcentajeTotal = round((100/65000)*$totales[0]->TotalSolicitudes,2);

        Self::crearDiapositiva($objPHPPresentation,'/archivos/valeGeneral.png',1,$totales[0]->TotalSolicitudes, $porcentajeTotal,$totales[0]->TotalValidaciones,$totales[0]->TotalAprobados,$totales[0]->TotalRechazados,null);

        for($i=0;$i<count($res);$i++){

            $porcentaje = round((100/$res[$i]->Apoyos)*$res[$i]->Solicitudes,2);
            Self::crearDiapositiva($objPHPPresentation,'/archivos/valeRegional.png',0,$res[$i]->Solicitudes,$porcentaje,$res[$i]->EnValidacion,$res[$i]->Aprobados,$res[$i]->Rechazados,$res[$i]->Region);    
        }

        $oWriterPPTX = IOFactories::createWriter($objPHPPresentation, 'PowerPoint2007');    
    ...
    public static function obtenerDatosVales($tipo){
        ...
        $table1 = "(select Region, count(idMunicipio) Municipios, sum(Recurso) Recurso, sum(Apoyos) Apoyos From meta_municipio GROUP BY Region) R";
        $table2 = "(select MN.SubRegion AS Region,  count(V.id) Solicitudes FROM vales V JOIN et_cat_municipio MN ON V.idMunicipio = MN.Id JOIN vales_status VS ON V.idStatus = VS.id group by MN.SubRegion) V";
        $table3 = "(select MN.SubRegion AS Region,  count(V.id) PorValedar FROM vales V JOIN et_cat_municipio MN ON V.idMunicipio = MN.Id JOIN vales_status VS ON V.idStatus = VS.id where VS.id=1 group by MN.SubRegion) PV";
        $table4 = "(select MN.SubRegion AS Region,  count(V.id) EnValidacion FROM vales V JOIN et_cat_municipio MN ON V.idMunicipio = MN.Id JOIN vales_status VS ON V.idStatus = VS.id where VS.id=6 group by MN.SubRegion) EV";
        $table5 = "(select MN.SubRegion AS Region,  count(V.id) Aprobados FROM vales V JOIN et_cat_municipio MN ON V.idMunicipio = MN.Id JOIN vales_status VS ON V.idStatus = VS.id where VS.id=5 group by MN.SubRegion) AC";
        $table6 = "(select MN.SubRegion AS Region,  count(V.id) Rechazados FROM vales V JOIN et_cat_municipio MN ON V.idMunicipio = MN.Id JOIN vales_status VS ON V.idStatus = VS.id where VS.id=4 group by MN.SubRegion) RC";
        
        if ($tipo == 1) $select = "R.Region, R.Municipios, R.Recurso, R.Apoyos, V.Solicitudes, EV.EnValidacion, AC.Aprobados, RC.Rechazados";
        else $select = "SUM(R.Recurso) as TotalRecurso, SUM(R.Apoyos) as TotalApoyo, SUM(V.Solicitudes) as TotalSolicitudes, SUM(EV.EnValidacion) as TotalValidaciones, SUM(AC.Aprobados) as TotalAprobados, SUM(RC.Rechazados) as TotalRechazados";
    ...
    public static function crearDiapositiva($objPHPPresentation,$ruta,$first,$datoSolicitudes,$datoPorcentaje,$datoValidaciones,$datoAprobados,$datoRechazados,$datoRegion){
        // Create slide
        if($first == 1) $currentSlide = $objPHPPresentation->getActiveSlide();
        else $currentSlide = $objPHPPresentation->createSlide();
        ...
        $shape->setName('PHPPresentation logo')
            ...
            Self::crearShapeText($currentSlide,$datoSolicitudes,250,360,true,'F3399FF0',60,160,40);
            Self::crearShapeText($currentSlide,$datoPorcentaje.'%',680,380,true,'F3399FF0',60,185,40);
            ...
            Self::crearShapeText($currentSlide,$datoAprobados,578,342,false,'FFFFFF',30,52,10);
            Self::crearShapeText($currentSlide,$datoValidaciones,578,380,false,'FFFFFF',30,52,10);
            Self::crearShapeText($currentSlide,$datoRechazados,578,418,false,'FFFFFF',30,52,10);
            Self::crearShapeText($currentSlide,'0',355,480,true,'F0000660',60,185,30);
            Self::crearShapeText($currentSlide,'0%',693,480,true,'F0000660',60,185,30);

            if(!is_null($datoRegion)) Self::crearShapeText($currentSlide,$datoRegion,480,280,false,'F0000660',30,75,12);
...
// funcion para generar los textos en el pptx
    public static function crearShapeText($currentSlide,$texto,$coorX,$coorY,$bold,$color,$height,$width,$size){

...
class ValesDocumentacionController extends Controller
{
        ...
        try {
            $res = DB::table('vales_documentacion')
                ->select(
                    ...
                    DB::raw(
                        'LPAD(HEX(vales_documentacion.id),6,0) as ClaveUnica'
                    ),
                    DB::raw(
                        'concat_ws(" ",vales_documentacion.Nombre, vales_documentacion.Paterno,vales_documentacion.Materno) as FullName'
                    ),
                ...
                ->leftJoin(
                    'vales_status_documentacion',
                    'vales_status_documentacion.id',
                    '=',
                    'vales_documentacion.idStatusDocumentacion'
                )
                ->leftJoin(
            ...
            if (isset($parameters['Propietario'])) {
                ...
                $res->where(function ($q) use ($valor_id) {
                    $q
                        ->where('vales_documentacion.UserCreated', $valor_id)
            ...
            if (isset($parameters['Folio'])) {
                ...
                $res->where(
                    DB::raw('LPAD(HEX(vales_documentacion.id),6,0)'),
                    'like',
                    '%' . $valor_id . '%'
                );
            }
        ...
        try {
            ...
            $res = DB::table('vales')
                ->select(
                    ...
                    DB::raw(
                        'concat_ws(" ",vales.Nombre, vales.Paterno,vales.Materno) as FullName'
                    ),
                    'vales.TelRecados',
                    'vales.Compania',
                    'vales.TelFijo',
    ...
    function setValesDocumentacion(Request $request)
    {
        ...
        $posible_registro = DB::table('vales_documentacion')
            ...
            ->where('vales_documentacion.id', '=', $parameters['id'])
            ->first();
        ...
        try {
            ...
            for ($i = 0; $i < count($res); $i++) {
                ...
                for ($x = 0; $x < count($res_1); $x++) {
                    if (
                        $res[$i]->UserOwned === $res_1[$x]->UserOwned &&
